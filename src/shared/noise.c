...
#define SMOOTHSTEP01(x) (3*(x)*(x)-2*(x)*(x)*(x))
...
float noise_2d(float sx, float sy, int seed) {
    ...
    v1 = (p1+(p2-p1)*SMOOTHSTEP01(sx-x));
    v2 = (p3+(p4-p3)*SMOOTHSTEP01(sx-x));
    v = v1+(v2-v1)*SMOOTHSTEP01(sy-y);
    return (v > -1 ? (v < 1 ? v : 1) : -1);
}

#define UNROLL8(s) i = 0;s;i = 1;s;i = 2;s;i = 3;s;i = 4;s;i = 5;s;i = 6;s; \
                   i = 7;s
...
float noise_3d(float sx, float sy, float sz, int seed) {
    ...
    v1 = (p[0]+(p[1]-p[0])*SMOOTHSTEP01(sx-x));
    v2 = (p[2]+(p[3]-p[2])*SMOOTHSTEP01(sx-x));
    v3 = v1+(v2-v1)*SMOOTHSTEP01(sy-y);
    v4 = (p[4]+(p[5]-p[4])*SMOOTHSTEP01(sx-x));
    v5 = (p[6]+(p[7]-p[6])*SMOOTHSTEP01(sx-x));
    v6 = v4+(v5-v4)*SMOOTHSTEP01(sy-y);
    v = v3+(v6-v3)*SMOOTHSTEP01(sz-z);
    return (v > -1 ? (v < 1 ? v : 1) : -1);
}
